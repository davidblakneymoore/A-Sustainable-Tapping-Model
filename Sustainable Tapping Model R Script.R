...
# 1. The tree is perfectly circular in cross section (and, by extension, it is
# perfectly circular in cross section at all heights, with each cross section
# being perfectly centered on each other and with no vessel meandering).

# 2. Each annual increase in the tree's radius is less than or equal to the
# length of the cylindrical portion of the tap hole (which does not include the
# most proximal part of the tap hole that is tapered and that was created by
# the tip of the drill bit).

# 3. All tap holes are made at the same exact height on the tree.

# 4. The heartwood depth is always greater than the tap hole depth where the
# tap hole is installed.

# 5. Tap holes are made in a perfectly radial direction and parallel with the
# ground and they are directed perfectly to the center of the tree.

# 6. Tap holes are made in a purely radial direction (there is no axial
# component to the tap hole direction).
...
# Tapping guidelines may suggest minimum tappable diameters of either 10 in or
# 12 in.
...
# I'm assuming a 2-in tap hole depth.
...
How_Should_Growth_Be_Modeled <- "Annual Radial Growth"
# The 'How_Should_Growth_Be_Modeled' argument can take two values ('Annual
# Basal Area Increment' and 'Annual Radial Growth').
Where_the_Figure_is_Being_Made <- "Hard Drive"
...
if (How_Should_Growth_Be_Modeled == "Annual Radial Growth") {
...
} else if (How_Should_Growth_Be_Modeled == "Annual Basal Area Increment") {
  # I could use a lognormal distribution or a uniform distribution
  # Here's how to use a lognormal distribution
...
# (https://mapleresearch.org/wp-content/uploads/wilmot_diameter.pdf). I also
# assumed that sugar maple tree ring width follows a lognormal distribution. My
...
# # Here's some stuff I should include when I actually write a function for this
# # algorithm.
# 
# if ((Tap_Hole_Drill_Bit_Point_Angle > (8 * pi / 9)) | (Tap_Hole_Drill_Bit_Point_Angle < (pi / 2))) {
#   stop ("The 'Tap_Hole_Drill_Bit_Point_Angle' argument must be between '(pi / 2)' and '(8 * pi / 9)', inclusive.")
# }
# # As it's written now, this function cannot handle drill bit point tips below
# # 90 ˚ or greater than 180 ˚. The 180-˚ case probably isn't important, but the
# # 90-˚ case could be. See the following website for information on how to make
# # this portion of the code better
# # (https://stackoverflow.com/questions/79308514/determining-which-points-on-the-perimeter-of-a-circle-fall-between-two-other-poi).
# if (Distance_Between_Fitted_Points >= (Tap_Hole_Width / 2)) {
#   stop ("The 'Distance_Between_Fitted_Points' argument must be less that half of the 'Tap_Hole_Width' argument in order for the function to work.")
# }
# if (Tap_Hole_Depth >= (Minimum_Tapping_Tree_Diameter / 2)) {
#   stop ("The 'Tap_Hole_Depth' argument must be less that half of the 'Minimum_Tapping_Tree_Diameter' argument.")
# }
...
# Here's a little helper function for calculating areas of overlap.
...
if (Where_the_Figure_is_Being_Made == "Hard Drive") {
  tiff(paste0(Working_Directory, "/Sustainable Tapping Model Figure.tiff"), height = 2250, width = 2000)
...
repeat {
  ...
  if (Radii[i] > (Minimum_Tapping_Tree_Diameter / 2)) {
    ...
    if (Has_the_First_Tap_Been_Installed_Yet == TRUE) {
      if (k == 2) {
        Point_in_Polygon_Algorithm_1 <- as.data.frame(sapply(seq_len(nrow(Tap_Hole_Polygon_Coordinates[[k]])), function (x) {
          sapply(seq_len((length(Tap_Hole_Wound_Polygon_Coordinates)) - 1), function (y) {
            sp::point.in.polygon(Tap_Hole_Polygon_Coordinates[[k]][x, 1], Tap_Hole_Polygon_Coordinates[[k]][x, 2], Tap_Hole_Wound_Polygon_Coordinates[[y]]$Horizontal_Coordinates, Tap_Hole_Wound_Polygon_Coordinates[[y]]$Vertical_Coordinates)
          })
        }))
      } else if (k > 2) {
        Point_in_Polygon_Algorithm_1 <- as.data.frame(t(sapply(seq_len(nrow(Tap_Hole_Polygon_Coordinates[[k]])), function (x) {
          sapply(seq_len((length(Tap_Hole_Wound_Polygon_Coordinates)) - 1), function (y) {
            sp::point.in.polygon(Tap_Hole_Polygon_Coordinates[[k]][x, 1], Tap_Hole_Polygon_Coordinates[[k]][x, 2], Tap_Hole_Wound_Polygon_Coordinates[[y]]$Horizontal_Coordinates, Tap_Hole_Wound_Polygon_Coordinates[[y]]$Vertical_Coordinates)
          })
        })))
      ...
      Point_in_Polygon_Algorithm_2 <- lapply(seq_len((length(Tap_Hole_Wound_Polygon_Coordinates)) - 1), function (x) {
        sapply(seq_len(nrow(Tap_Hole_Wound_Polygon_Coordinates[[x]])), function (y) {
          sp::point.in.polygon(Tap_Hole_Wound_Polygon_Coordinates[[x]][y, 1], Tap_Hole_Wound_Polygon_Coordinates[[x]][y, 2], Tap_Hole_Polygon_Coordinates[[k]]$Horizontal_Coordinates, Tap_Hole_Polygon_Coordinates[[k]]$Vertical_Coordinates)
        })
      })
      ...
      if (length(Overlapping_Polygon_Numbers) == 0) {
        ...
        Data_Frame[i, ]$Area_of_Overlap_Between_This_Tap_Hole_and_Other_Nonconductive_Wood_Columns <- Calculating_Areas_of_Overlap_Function(Polygon_of_Interest = Tap_Hole_Polygon_Coordinates[[k]], Other_Polygons = Tap_Hole_Wound_Polygon_Coordinates[Overlapping_Polygon_Numbers])
