...
Tap_Hole_Spacing_Measurement_Location <- "External" # This argument can take values of "Internal" or "External"
Minimum_Tap_Hole_Spacing <- 5 # cm
# This 'Minimum_Tap_Hole_Spacing' argument determines the minimum distance
# between tap holes. If the 'Tap_Hole_Spacing_Measurement_Location' argument is
# '"Internal"', then the spacing refers to the minimum distance between a 
# previous year's column of nonconductive wood and a current year's tap hole.
# If the 'Tap_Hole_Spacing_Measurement_Location' argument is '"External"', then
# the spacing refers to the minimum distance around a tree's circumference (an
# arc length). I originally included this parameter (using the '"Internal"'
# method for the 'Tap_Hole_Spacing_Measurement_Location' argument) in part
# because the point-in-polygon algorithm I use sometimes returns a false
# positive due to rounding. (Obviously, in practice, people use external
# distances and not internal distances.)
...
repeat {
  ...
  if (Radii[i] > (Minimum_Tapping_Tree_Diameter / 2)) {
    ...
    if (Has_the_First_Tap_Been_Installed_Yet == FALSE) {
      ...
      k <- k + 1
      if (Tap_Hole_Spacing_Measurement_Location == "Internal") {
      ...
      } else if (Tap_Hole_Spacing_Measurement_Location == "External") {
        Angles[k] <- Angles[(k - 1)] + (Minimum_Tap_Hole_Spacing / Radii[i])
      }
    }
